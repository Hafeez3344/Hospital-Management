    ...
    <>
      <div className="space-y-6 pb-12">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button onClick={() => setOpen(true)} className="px-4 py-2 text-xs font-semibold bg-purple-600 hover:bg-purple-700 text-white rounded-xl transition-colors shadow-md shadow-purple-900/20 flex items-center gap-1.5 self-start sm:self-center">
                  ...
                  <tr key={lab.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <button
                        ...
                        className="px-3 py-1 text-xs font-bold text-purple-700 bg-purple-50 hover:bg-purple-100 rounded-lg border border-purple-200"
                      >
                        Enter Results
                      </button>
                    </td>
      ...
      <QuickActionModal isOpen={open} onClose={() => setOpen(false)} type="new-lab-order" />
    </>
  );
}
