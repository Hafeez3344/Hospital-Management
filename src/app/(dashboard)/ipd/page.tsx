    ...
    <>
      <div className="space-y-6 pb-12">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button onClick={() => setOpen(true)} className="px-4 py-2 text-xs font-semibold bg-teal-600 hover:bg-teal-700 text-white rounded-xl transition-colors shadow-md shadow-teal-900/20 flex items-center gap-1.5 self-start sm:self-center">
              ...
              {bed.isOccupied ? (
                ...
                <div className="border-t border-emerald-200/60 pt-2 flex items-center justify-between text-xs">
                  ...
                  <button
                    ...
                    className="text-[11px] font-bold text-emerald-700 bg-emerald-100 hover:bg-emerald-200 px-2 py-0.5 rounded"
                  >
                    Assign Bed
                  </button>
                </div>
      ...
      <QuickActionModal isOpen={open} onClose={() => setOpen(false)} type="new-ipd-admission" />
    </>
  );
}
